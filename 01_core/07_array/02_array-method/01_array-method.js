...
//Array.prototype.push : 배열의 맨뒤에 요소 추가
//Array.prototype.pop : 배열의 맨 뒤에 요소 제거

const chineseFood = ['짜장면', '짬뽕', '우동'];

console.log(`push 전 chinessFood : ${chineseFood}`);

chineseFood.push('탕수육');
chineseFood.push('양장피');

console.log(`push 후 chinessFood : ${chineseFood}`);
console.log(`chinessFood.pop() : ${chineseFood.pop()}`);
console.log(`chinessFood.pop() : ${chineseFood.pop()}`);
console.log(`chinessFood.pop() : ${chineseFood.pop()}`);
...
const mix2 = idol1.concat(idol2, idol3);
console.log(`idol1 기준으로 idol2 배열을 concat : ${mix}`);
console.log(`idol1 기준으로 idol2, idol3 배열을 concat : ${mix2}`);
...
const snackList = ['사탕', '초콜릿', '츄잉껌', '맛있는 과자'];

console.log(`snackList.join() : ${snackList.join()}`);
console.log(`snackList.join('/') : ${snackList.join('/')}`);

// Array.prototype.reverse : 배열의 순서를 뒤집는다.
